...
func TestZipDocExtractorsBoundDecompressedEntryStream(t *testing.T) {
	...
	// the old unbounded path provably exceeds the cap, small enough
	...
	cases := []struct {
		...
		// Each body is WELL-FORMED: against the old unbounded code
		// the parse succeeds and the full bomb comes back as chunks
		...
		},
		{
			...
			ex:    NewODPExtractor(),
			entry: "content.xml",
			body:  `<?xml version="1.0"?><office><body><page><p>` + bomb + `</p></page></body></office>`,
		},
		{
			name:  "pptx",
			ex:    NewPPTXExtractor(),
			entry: "ppt/slides/slide1.xml",
			body:  `<?xml version="1.0"?><sld><txBody><p><t>` + bomb + `</t></p></txBody></sld>`,
		},
		{
			name:  "epub",
			ex:    NewEPUBExtractor(),
