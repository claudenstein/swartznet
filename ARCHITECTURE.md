...
2. **Identity is persistent and load-bearing.** `identity.key` (mode exactly 0600) backs publisher reputation and signing; losing it loses standing. Never regenerate implicitly. *Design constraint:* one `identity` package with exact-0600 enforcement, seed→pubkey re-derivation on load, and auto-create **only** at the default XDG path — the rule is path-based, so an explicit `--identity` naming a non-default path is load-only while naming the default path behaves like the default. The caller computes `allowCreate` by comparing the (cleaned) configured path against `config.Default().IdentityPath`, and `Load`'s create branch is the single enforcement site.
...
- **Key types:** `Identity`, `Signer`, `Load(path string, allowCreate bool) (*Identity, error)`, `PublicKeyHex()`. The identity leaf imports stdlib only, so it cannot resolve the XDG default itself: the daemon computes `allowCreate` by comparing the configured path against `config.Default().IdentityPath`, and `Load`'s create branch (missing file + `allowCreate`) is the single enforcement site — atomically, closing the legacy stat-then-create TOCTOU. The rule is path-based: an explicit `--identity` naming a non-default path is load-only; naming the default path behaves like the default.
...
- **Boundary (architectural law, SPEC §5.8):** **zero subsystem imports.** Every collaborator is a narrow interface declared here; every DTO is re-declared and JSON-tagged locally; a nil collaborator returns 503. No auth concept exists. A 1 MiB body cap wraps the CSRF/DNS-rebind guard (Origin-then-Referer, literal `localhost` trusted, other DNS names rejected) wraps the mux — `bodylimit ⊃ CSRF ⊃ mux`, matching SPEC §5.9 and the legacy order (the cap only swaps `r.Body`, so a CSRF-rejected request never reads its body); the services readout comes from an injected `ServicesReporter` bound to `capability.Announced`.
