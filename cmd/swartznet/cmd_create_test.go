...
func TestCmdCreateSignHappyPath(t *testing.T) {
	...
	// An explicit --identity is load-only, so the key file must
	// already exist; writeTestKey seals a valid 0600 ed25519 key.
	idPath := writeTestKey(t)
	outPath := filepath.Join(dir, "out.torrent")
...
// TestCmdCreateSignMissingExplicitIdentityFailsClosed is the
// regression test for the silent-mint bug: --sign --identity with a
// missing file must fail (non-zero exit) and must NOT create a fresh
// untrusted key at that path. Before the fix this called
// identity.LoadOrCreate, which minted a brand-new key and signed the
// .torrent with an unknown pubkey.
func TestCmdCreateSignMissingExplicitIdentityFailsClosed(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	src := filepath.Join(dir, "src.bin")
	if err := os.WriteFile(src, []byte("x"), 0o644); err != nil {
	...
	outPath := filepath.Join(dir, "out.torrent")
	idPath := filepath.Join(dir, "missing.key")
	...
	if code == exitOK {
		t.Errorf("missing-explicit-identity exit = %d, want non-zero (fail closed)", code)
	}
	if _, err := os.Stat(idPath); err == nil {
		t.Error("cmdCreate minted a key at a missing explicit --identity path (should fail closed)")
	}
}
...
// branch. Spawn cmdCreate --seed in a goroutine, give it a moment to
// enter the seeding loop, then SIGINT the process. The seeding loop
// should observe ctx.Done() and map the cancellation to
// exitInterrupt (130) — consistent with cmdAdd — so scripts can tell
// an interrupted seed from a clean exit by exit code.
func TestCmdCreateSeedThenSigint(t *testing.T) {
	...
	case code := <-done:
		if code != exitInterrupt {
			t.Errorf("cmdCreate --seed exit = %d, want exitInterrupt", code)
		}
