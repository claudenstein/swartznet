...
# ======================================================================
# Part 1 — default (index-on) node: live mask 0x2FD, publisher on
# ======================================================================
...
[ "$(cap "$ADDR" services)" = "00000000000002fd" ] && ok "GET /capabilities services is the live mask 0x2FD" || fail "cap services = $(cap "$ADDR" services)"
# Defect #1: /aggregate renders the LIVE mask, not the static 0x2ED.
S=$(agg "$ADDR")
{ [ "$S" = "00000000000002fd" ] && [ "$S" != "00000000000002ed" ]; } && ok "/aggregate services is live (0x2FD), not the static 0x2ED" || fail "/aggregate services = $S"
...
[ "$(cap "$ADDR" services)" = "00000000000002f9" ] && ok "services dropped only the FileHits bit (0x2F9)" || fail "services after file toggle = $(cap "$ADDR" services)"
...
[ "$(agg "$ADDR")" = "00000000000002f0" ] && ok "downgrade honored: /aggregate → 0x2F0 (legacy would still show static 0x2ED)" || fail "downgrade /aggregate = $(agg "$ADDR")"
...
# ======================================================================
# Part 2 — --no-index cascade zeroes the Publisher bit (mask 0x2ED)
# ======================================================================
...
[ "$(cap "$ADDR" services)" = "00000000000002ed" ] && ok "--no-index mask is 0x2ED (bit 4 clear)" || fail "no-index services = $(cap "$ADDR" services)"
[ "$(agg "$ADDR")" = "00000000000002ed" ] && ok "--no-index /aggregate matches (live)" || fail "no-index /aggregate = $(agg "$ADDR")"
